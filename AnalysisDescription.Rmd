...
lapply(compAGE, function(eachList){
knitr::kable(as.data.table(eachList))
})
```
...
lapply(statAGE, function(eachList){
knitr::kable(as.data.table(eachList))
})
lapply(statSEX, function(eachList){
knitr::kable(as.data.table(eachList))
})
...
```{r tables with p.value, echo=FALSE, eval=TRUE}
knitr::kable(as.data.table(phAGE))
knitr::kable(as.data.table(phBIOME))
```
**NOTE:** The first three columns are the p.values of their respective comparisons (indicated in the ), while the last three columns are the means so it is possible to know which ones are higher and lower than the others.
