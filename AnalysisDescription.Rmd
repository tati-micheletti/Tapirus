...
# Still need to figure out how to make bthe table. Not sure table "final.table" as it is now (with columns such as SEX_GROUPS, BIOME_GROUPS and AGE_GROUPS) is good...
# Need to show the tests for each one of these (BIOME, SEX, AGE). Maybe sequentially subsetting the biomes, and sex, testing each age, then ignoring age and looking only into sex, and so on?... Think about it

variables <- as.character(colnames(hembio.dt))


test_groups <- data.table(PARAM = rep(c
                                      ("PARAMETRIC", "NON-PARAMETRIC"), each = 2), GROUP = c(2,3,2,3), TEST = c("T-Test", "ANOVA", "Mann-WhitneyU","Kruskal-Wallis"))
