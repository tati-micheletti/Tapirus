...
vari.table <- data.table(Variables=variables)
```

## The following tables show the variables that have at least one value for sub-adults and juveniles in comparison to adults 
### Atlantic Forest
Female groups of Atlantic Forest:
``` {r compFemales, echo=TRUE, eval=TRUE}
...
At.F.Age <- merge(vari.table,At.F.Age, all.y=TRUE)
knitr::kable(At.F.Age)
At.F.Age$enoughSamples <- NA

# Need to redo these tables as it is also including Age and Sex (and not sure why acidourico starts with lower case)
...
At.MJuv <- At.M[Age=="Juvenile"&count>1]$variable
...
Cr.MJuv <- Cr.M[Age=="Juvenile"&count>1]$variable
