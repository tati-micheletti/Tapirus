...
AGE.table <- lapply(biome, function(biome){
  TEST <- lapply(sex, function(sex){
      TEST <- apply(compAGE[[biome]][[sex]], 1, function(rows){
         TEST <- ifelse(is.na(rows["test"]), NA, #compAGE[[biome]][[sex]]$TEST <-  
                 ifelse(rows["test"]=="T",{
...
# NOW I NEED TO BIND BOTH LISTS: AGE.tables and compAGE. AGE.table needs to be a column in compAGE dataframes.
# maybe use cbind list or some similar
compAGE <- cbindlist
