...
hembio.dt <- as.data.table(hembio[,-"ID"])
variables <- as.character(colnames(hembio.dt))
sum_groups <- hembio.dt[, .N, by = c("Biome", "Sex", "Age")] #NOT YET WHAT I NEED. Need to have this for each variable! CHECK https://stackoverflow.com/questions/29633168/tabulate-responses-for-multiple-columns-by-grouping-variable-with-dplyr MAYBE HELPS.
...
for (i in 1:nrow(final.table)){
  ...
  if (!(final.table[i,"VARIABLES"] %in% homo
