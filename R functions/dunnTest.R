...
dunn <- function(x=NA, g=NA, method="none", kw=TRUE, label=TRUE, wrap=FALSE, table=TRUE, list=FALSE, rmc=FALSE, alpha=0.05, altp=FALSE) {
  ...
  tpad <- function(n=1, pad=" ") {
    ...
    return(invisible(paste0(rep(x=pad,times=n),collapse="")))
  }
  ...
  zformat <- function(z) {
    ...
    return(invisible(paste0(sign,leftdigits,".",rightdigits)))
  }
  ...
  centertext <- function(text,width=80,lmargin=2,rmargin=2) {
    ...
    if (buff%%2 == 0) {
      return(invisible(paste(paste(rep(" ",buff/2),collapse=""),text,paste(rep(" ",buff/2),collapse=""),"\n",collapse="")))
    }
    else {
      return(invisible(paste(paste(rep(" ",1+(buff-1)/2),collapse=""),text,paste(rep(" ",(buff-1)/2),collapse=""),"\n",collapse="")))
    }
  }
  ...
  alphabetize.factor <- function(x) {
    return(invisible(factor(sort(c(as.character(x))))))
  }
